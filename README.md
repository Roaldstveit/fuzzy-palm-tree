## Innhold
{:toc}


## Velkommen

Dette er framsida

[Gå til Brukerveiledning](Brukerveiledning.md)

## fuzzy-palm-tree

To lange sider

## Ingen begrensninger

`#000000 `


hei hei 🇳🇴

🍕


### Liste over ingen ting fornuftig

<details>
  <summary>Liste over linjer</summary>
  
- Fyste linje - Andre linje - Tredje linje
- Andre linje
- **Tredje linje**
- _Fjerde linje_
- **Femte _linje_ satt**
</details>

> Nytt kapitell kjem no

## Totalt frislepp


## Ny galskap

## Pål sine båt senarioer 

|Type båt | Lengde |  Pål's vurdering |
|:--------:|:--------:| :--------:| 
|Skjærgårsjeep|14 fot|Ypperlig|
|Cabincruiser|28 fot|Plastjolle|
|Katamaran|50 fot|Støttehjulsbåt|

## Pål på tur

# Begreper

|**Begrep**|**Forklaring**|**Referanse**|
|:------|:------|:------|
|Transaksjon|Utbetaling i Reskontro, overført til og lastet inn i RAY| |


## Begreper relatert til A-ordningen

|**Begrep**|**Forklaring**|**Referanse**|
|:------|:------|:------|
|A-melding|Melding for rapportering av ytelser til A-ordningen| |
|A-melding-tilbakemelding	| Tilbakemelding på en A-melding| Er denne fortsatt gjeldande?|
|Ytelsesperiode (Funksjonell periode) Dato_FOM og Dato_TOM | Den faktiske perioden en ytelse er opparbeidet for. Når A-melding har gyldig etterbetalingsperiode er det denne verdien som blir rapportert som periode| |
|Posteringsdato|Dato postert i Utbetalingsreskontro (UR)||
|Utbetalingsdato|Dato bruker har fått overført utbetaling til sin bankkonto. Begrep innført fra 1.1.2026, ikke benyttet 1.1.2015 til 31.12.2025||




## Årsoppgave
Hvert rapporteringsår gjelder fra og med 1.januar til og med 31.desember samme år. Det skal utarbeides en oppstilling av alle utbetalinger og trekk gjennomført til hver enkelt bruker. Oppstillingen heter Årsoppgave. Vi skiller på to typer årsoppgaver.

|**Begrep**|**Forklaring**|**Referanse**|
|:------|:------|:------|
|Hovedoppgave|Årsoppgaven som sendes for et gitt år| |
|Endringsoppgave|Utarbeides dersom man har gjort endringer på et avsluttet rapporteringsår. Beløp i endringsoppgave oppfattes som et tillegg til beløp i hovedoppgaven| |

### Sidespor
Enkel endring <details> <Summary> Enkeltspor </Summary> To tog, på ei linje, eller to linjer med eit tog. </details> 
